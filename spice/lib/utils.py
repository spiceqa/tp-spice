...
class CommandsLinux(Commands):
    ...
    def firefox_auto_open_vv(self):
        ...
        for o in opts:
            self.vm.info("Remove old value %s from Firefox profile: %s", o,
                         user_js)
            cmd = "sed -i -e '/%s/d' %s" % (o, user_js)
            self.ssn.cmd(cmd)
        line = 'pref("browser.helperApps.neverAsk.openFile", "application/x-virt-viewer");'
        cmd = "echo %s >> %s" % (line, user_js)
        self.vm.info("Add new line %s to Firefox profile: %s", line, user_js)
        self.ssn.cmd(cmd)
